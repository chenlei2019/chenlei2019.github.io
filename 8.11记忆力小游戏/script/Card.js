...
function Card(imgIndex){
    this.status = "back";  //卡片状态：back背面，front正面，clear已经消除
 ...
 * 根据传入的新状态，改变状态
 */
Card.prototype.setStatus = function(newStatu){
    if(newStatu === "back"){
        this.dom.className = "";//背面是没有类样式的
    }
    else if(newStatu === "front"){
        this.dom.className = "front";//前面
    }
    else if(newStatu === "clear"){
        this.dom.className = "clear";//清理了
    }
    else {
        throw new Error("状态不正确");//
    }
    this.status = newStatu;
    var that = this;
    //动画什么时候结束
    this.dom.addEventListener("transitionend",function(){
        //要做一些事，但是，这里，不知道要做什么事
